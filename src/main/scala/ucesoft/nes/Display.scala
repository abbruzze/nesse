...
import ucesoft.nes.{Clock, Log, NESComponent, NESComponentType}
...
object Display:
  trait DisplaySizeChangedListener:
    def displaySizeChanged(width:Int,height:Int,xRatio:Double,yRatio:Double): Unit

class Display(width: Int, height: Int, title: String, frame: JFrame, clk:Clock = Clock.systemClock) extends JComponent with MouseMotionListener with MouseListener with NESComponent {
  val componentID = "Display"
  override val componentType = NESComponentType.VIDEO
  ...
  private[this] val ptrDisplayMem = normalDisplayMem
  ...
  private[this] var displaySizeChangedListener : Display.DisplaySizeChangedListener = _

  addMouseMotionListener(this)
  addMouseListener(this)

  def setDisplaySizeChangedListener(l:Display.DisplaySizeChangedListener): Unit = displaySizeChangedListener = l

  def blankScreen(): Unit =
    java.util.Arrays.fill(normalDisplayMem,0xFF000000)
  ...
  def notifyDisplaySizeChange(): Unit = displaySizeChangedListener.displaySizeChanged(dimension.width,dimension.height,math.round(zoomFactorX),math.round(zoomFactorY))
  ...
  override final def paint(g: Graphics) : Unit = {
    if (dimension.width != getWidth || dimension.height != getHeight) {
      ...
      if displaySizeChangedListener != null then
        displaySizeChangedListener.displaySizeChanged(dimension.width,dimension.height,math.round(zoomFactorX),math.round(zoomFactorY))
    ...
    if (drawRasterLine) {
      ...
      if (clip == null) g.fillRect(0, (rasterLine * zoomFactorY).toInt, dimension.width, zoomFactorY.toInt)
      else
