...
import ucesoft.nes.Chip
import ucesoft.nes.trace.TraceListener
import ucesoft.nes.NESComponentType
import Bits.*

object CPU65xx {
  class CPUJammedException(val cpuID:ChipID,val pcError:Int) extends Exception
  ...
  def disassemble(mem:Memory,address: Int) = {
    ...
    val (ind, len) = op._2 match {
      ...
      case REL =>
        ...
        ("$" + hex4(target), 1)
      case ABS => ("$" + hex4(word(address + 1, mem)), 2)
      case ABX | ABY => ("$" + hex4(word(address + 1, mem)) + "," + (if (op._2 == ABX) "X" else "Y"), 2)
      case IND => ("($" + hex4(word(address + 1, mem)) + ")", 2)
  ...
  def make(mem:Memory,id:ChipID = ChipID.CPU) : CPU65xx = new CPU6502(mem,id)
}

trait CPU65xx extends Chip with TraceListener {
  override val componentID = "6502"
  override val componentType = NESComponentType.CPU
