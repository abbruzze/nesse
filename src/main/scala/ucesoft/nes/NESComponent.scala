...
enum NESComponentType:
  case CPU, PPU, APU, MEMORY, CHIP, VIDEO, AUDIO, JOYSTICK, MOTHERBOARD, CARTRIDGE
  
object NESComponent:
  def loadMemory[T](mem:Array[T],in:ObjectInputStream) : Unit = {
    val ram = in.readObject.asInstanceOf[Array[T]]
    if (ram.length != mem.length) throw new IOException(s"ROM/RAM length mismatch. Expected ${mem.length}, found ${ram.length}")
    Array.copy(ram,0,mem,0,ram.length)
  }

trait NESComponent {
  val componentID : String
  val componentType : NESComponentType
  protected val properties = new Properties
  private[this] val _components = new ListBuffer[NESComponent]

  def getProperties = properties

  final def add(c:NESComponent) : Unit = {
  ...
  final def change(oldComponent:NESComponent, newComponent:NESComponent) : Unit = {
  ...
  final def remove(c:NESComponent) : Unit = {
    _components -= c
  }
  ...
  final def printComponentsTree  : Unit = {
    def print(c:NESComponent, ind:Int) : Unit = {
