...
object Log {
  ...
  private var showClockCycle = true
  ...
  def setShowClockCycle(show:Boolean) : Unit = showClockCycle = show
  
  inline private def format(m:String) =
    if showClockCycle then "[%10d]\t%s".format(if (Clock.isAvailable) Clock.systemClock.currentCycles else 0,m)
    else m
